...
- [x] **T5.1** Add `Application` use case `SaveContractCommand` + handler that calls `ITajeerContractClient`, persists a minimal `Lease` row with `Status = PendingIssuance`. **Verify**: unit test with InMemory client + InMemoryDb confirms row written. ✅ 2026-05-23 (`Application/Leases/SaveContractCommand{,Handler}.cs` + `ILeaseRepository` port + `EfLeaseRepository`; new `Application.Tests` project — 5 tests cover happy path, vendor error, idempotency replay, cross-tenant key isolation, empty tenant guard).
- [x] **T5.2** Add `Domain.Lease` minimal entity (`Id`, `TenantId`, `CustomerId?`, `TajeerContractNumber?`, `IssuanceUrl?`, `Status`, `CreatedAt`, `UpdatedAt`). **Verify**: EF Core configuration compiles. ✅ 2026-05-23 (`Domain/Leases/{Lease,LeaseStatus}.cs` + `Infrastructure/Persistence/Configurations/LeaseConfiguration.cs` with unique filtered index on `(TenantId, TajeerContractNumber)`).
- [x] **T5.3** Generate initial EF migration `Init_Lease`. **Verify**: `dotnet ef migrations add Init_Lease` succeeds; migration SQL inspected. ✅ 2026-05-23 (`Persistence/Migrations/20260522232532_Init_Lease.cs`; `AutoLeaseNetDbContextFactory` is the design-time factory; local `dotnet-ef 8.0.5` tool manifest committed to `.config/dotnet-tools.json`).
- [x] **T5.4** Apply migration to ~~Dockerized SQL Edge~~ local SQL Server 2019 Developer. **Verify**: `dotnet ef database update` exits 0; `SELECT * FROM Leases` works. ✅ 2026-05-23 (`Applying migration '20260522232532_Init_Lease'. Done.`; verified via `sqlcmd -Q "SELECT MigrationId FROM __EFMigrationsHistory"`).
- [x] **T5.5** Add BFF endpoint `POST /api/v1/dev/save-contract` requiring `Idempotency-Key` header. **Verify**: missing header → 400; present → 202 with response body. ✅ 2026-05-23 (`services/bff/Endpoints/DevEndpoints.cs` — 400 on missing header, 422 on vendor error, 503 on transient infra, 202 with `{leaseId, tajeerContractNumber, issuanceUrl}` on success; `SaveContractEndpointTests` × 3 via `WebApplicationFactory`).
- [x] **T5.6** Wire idempotency via `Adapters.Cache.InMemory.AddInMemoryCache()` against the `IIdempotencyStore` port (24h TTL) keyed on `tenant + idempotency-key`. **Verify**: duplicate POST returns same cached response. ✅ 2026-05-23 (key namespace `tenant:{guid}:save-contract:{client-key}`; replay returns byte-identical JSON with `Tajeer.SaveCalls.Count == 1` — asserted at both handler and endpoint layers). _(Swap to Redis in §6 loop-back when Docker/Memurai available.)_
- [ ] **T5.7** **First real Save against Tajeer staging** from local BFF (Postman or curl with dev JWT stub header). **Verify**: 202 response with real `ContractNumber` + `IssuanceUrl`; row in `Leases` table with `PendingIssuance`. ⏳ awaiting manual run — full PowerShell + Invoke-RestMethod recipe placed in notes.md Day 5 section.
- [ ] **T5.8** Capture sanitized request/response in `notes.md` + diagram the call flow if anything surprised. **Verify**: notes updated. ⏳ template placed in notes.md Day 5 section; fill in after T5.7.
